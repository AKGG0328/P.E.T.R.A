...
def simple_calcium_process():
    ...
    # fool proof
    while True:
        choice_raw = input(f"\nType the number of the sheet (0 to {len(sheet_names) - 1}) or 'q' to quit: ").strip().lower()
        ...
        try:
            ...
            print(" Invalid input. Please type a standard number or 'q'.")
    ...
    parts = selected_sheet.split('-')
    if len(parts) > 2:
        base_name = "-".join(parts[:-1])
    else:
        base_name = selected_sheet
    folder_pattern = f"{base_name}*"
    matching_folders = [f for f in glob.glob(folder_pattern) if os.path.isdir(f)]

    if not matching_folders:
        print(f" Error: Could not find any folder starting with '{base_name}'")
        input("Press Enter to exit...")
        return
    target_folder = max(matching_folders, key=os.path.getmtime)
    rgn_path = os.path.join(target_folder, 'region.RGN')

    print(f"  -> Locating coordinates in: '{rgn_path}'")

    try:
        rgn_df = load_metafluor_rgn(rgn_path)
    ...
    output_filename = 'Data_Output.xlsx'
    if os.path.exists(output_filename):
        with pd.ExcelWriter(output_filename, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
            styled_df.to_excel(writer, sheet_name=selected_sheet, index=False)
        print(f" All time points processed! Appended as new sheet '{selected_sheet}' inside '{output_filename}'\n")
    else:
        # Create a brand new file (for the very first time you run this)
        with pd.ExcelWriter(output_filename, engine='openpyxl', mode='w') as writer:
            styled_df.to_excel(writer, sheet_name=selected_sheet, index=False)
        print(f" All time points processed! Created new file '{output_filename}' with sheet '{selected_sheet}'\n")
